...
describe('Fetch All Investments By PortfolioId', () => {
    ...
    it('should be able to fetch all investiments by portfolio id', async () => {
        ...
        if (result.isRight()) {
            ...
            expect(investments[0].quantity.getValue()).toBe(10)
            expect(investments[0].currentPrice.getAmount()).toBe(50)
            expect(investments[1].quantity.getValue()).toBe(20)
            expect(investments[1].currentPrice.getAmount()).toBe(100)
        }
    })
    ...
    it('should be able to paginated all investiments by portfolio id', async () => {
        ...
        if (result.isRight()) {
            ...
            expect(investments).toHaveLength(2)
            expect(investments[0].quantity.getValue()).toBe(210) // 21 * 10
            expect(investments[0].currentPrice.getAmount()).toBe(105) // 21 * 5
            expect(investments[1].quantity.getValue()).toBe(220) // 22 * 10
            expect(investments[1].currentPrice.getAmount()).toBe(110) // 22 * 5
        }
    })
