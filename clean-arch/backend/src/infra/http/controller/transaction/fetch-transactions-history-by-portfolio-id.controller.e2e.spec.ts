...
describe('Fecth Transactions History By PortfolioId (E2E)', () => {
    ...
    test('[GET] /transactions', async () => {
        ...
        const assetX = await assetFactory.makePrismaAsset()
        const assetXId = assetX.id.toValue().toString()

        const assetY = await assetFactory.makePrismaAsset()
        const assetYId = assetY.id.toValue().toString()
        ...
        await Promise.all([
            investmentFactory.makePrismaInvestment({
                assetId: assetX.id,
